...
void solve() {
  int N, M, K; loadVar(N, M, K);
  VI A(M), B(M); loadVec(M, A, B);
  VI P(K), H(K); loadVec(K, P, H);

  vector<set<int>> branches(N);
  rep(i, M) {
    int a = A[i] - 1;
    int b = B[i] - 1;
    branches[a].insert(b);
  ...
  VI dp(N);
  map<int, unordered_set<int>> ranks;
  rep(i, K) {
    int p = P[i] - 1;
    int h = H[i];
    dp[p] = h;
  ...
  for(auto itr = ranks.rbegin(); itr != ranks.rend(); itr++) {
    for(auto v: (*itr).second){
      ans.insert(v+1);
      for(auto w: branches[v]) {
        if(dp[w] < (*itr).first - 1) {
          ranks[dp[w]].erase(w);
          dp[w] = (*itr).first - 1;
          ranks[(*itr).first - 1].insert(w);
        }
      }
    }
  }
