...
void solve() {
  ...
  rep(i, Q) {
    LL f_sep = lower_bound(A.begin(), A.end(), L[i]) - A.begin();
    LL e_sep = lower_bound(A.begin(), A.end(), R[i]) - A.begin();
    LL idx = f_sep;
    ...
    if(f_sep % 2 == 0) total_sleep += A[f_sep] - start;
    if(e_sep % 2 == 0) total_sleep += end - A[e_sep - 1];

    total_sleep += SUM[e_sep -1] - SUM[f_sep];
    cout << total_sleep <<endl;
  }
}
...
#else
  const int n_testcase = 3;  // Don't change here!!
#endif
