...
void FindStars(vector<unordered_set<int>>& tree, queue<int>& ps, unordered_set<int>& used, map<int, int>& stars) {
  int i = ps.front(); ps.pop();

  int parent;
  for(int q: tree[i]) {
    if(used.count(q) != 0) continue;
    parent = q;
    break;
  }
  stars[tree[parent].size()] ++;
  used.insert(parent);
  ...
  if(!ps.empty()) FindStars(tree, ps, used, stars);
}

void solve() {
  int N; loadVar(N);
  VI U(N-1), V(N-1); loadVec(N-1, U, V);
  ...
  rep(i, N) {
    if(tree[i].size() == 1) {
      queue<int> qs;qs.push(i);
      unordered_set<int> used;
      FindStars(tree, qs, used, stars);
      break;
    }
  }
