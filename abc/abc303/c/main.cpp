...
void solve() {
  int N, M, H, K; loadVar(N, M, H, K);
  string S; loadVar(S);
  VI X(M), Y(M); loadVec(M, X, Y);
  
  unordered_map<pair<int, int>, int, HashPair> food;
  rep(i, M) {
    food[pair<int, int>(X[i], Y[i])] = 1;
  }
  ...
  for(char c: S) {
    ...
    if(H < K && food[pair<int,int>(x, y)] == 1) {
