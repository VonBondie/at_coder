...
void solve() {
  int N, K, Q; loadVar(N, K, Q);
  VLL X(Q), Y(Q); loadVec(Q, X, Y); 

  VLL A(N, -1);
  std::multiset<LL> use;
  std::multiset<LL> unuse;
  LL total = 0ll;
  ...
  rep(i, Q) {
    ...
    else if(A[x] > (*use.rbegin())) {
      total -= A[x];
      if(use.find(A[x]) != use.end()) use.erase(use.find(A[x]));
      A[x] = Y[i];
      unuse.insert(A[x]);
      total += *(unuse.rbegin());
      use.insert(*(unuse.rbegin()));
      LL tmp = *unuse.rbegin();
      int del_num = unuse.erase(*unuse.rbegin());
      rep(j, del_num) unuse.insert(tmp);
    }
    else {
      LL tmp2 = A[x];
      int del_num2 = unuse.erase(A[x]);
      rep(j, del_num2) unuse.insert(tmp2);
      A[x] = Y[i];
      if(A[x] > *use.rbegin()) {
        total+= A[x];
        use.insert(A[x]);
        unuse.insert(*use.rbegin());
        LL tmp = *use.rbegin();
        int del_num = use.erase(*use.rbegin());
        rep(j, del_num) use.insert(tmp);
      } else {
        unuse.insert(A[x]);
      }
    }
