...
void solve() {
  ...
  // vector<bool> used_H(W, false);
  // vector<bool> used_W(H, false);
  unordered_set<LL> used_H;
  unordered_set<LL> used_W;
  ...
  for(LL i = M-1; i>=0; i--) {
    if(T[i] == 1) {
        // if(used_W[A[i]]) continue;
        if(used_W.find(A[i]) != used_W.end()) continue;
        LL change = W - used_C;
        ...
        // if(used_H[A[i]]) continue;
        if(used_H.find(A[i]) != used_H.end()) continue;
        LL change = H - used_R;
        if(change == 0)continue;
        sum[X[i]] += change;
        sum[0] -= change;
