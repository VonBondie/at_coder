...
void solve() {
  ...
  segtree<unsigned long long, op, e>  costs_0(N);
  segtree<unsigned long long, op, e>  costs_1(N);

  rep(i, N) {
    const char should_0 = (i%2 ? '1' : '0');
    const char should_1 = (i%2 ? '0' : '1');
    costs_0.set(i, S[i] == should_0 ? 0 : C[i]);
    costs_1.set(i, S[i] == should_1 ? 0 : C[i]);
  }

  unsigned long long ans = ULONG_LONG_MAX;

  rep1(i, N-1) {
    const unsigned long long c_0 = costs_0.prod(0, i) + costs_1.prod(i, N);
    const unsigned long long c_1 = costs_1.prod(0, i) + costs_0.prod(i, N);
    const unsigned long long c = min(c_0, c_1);
