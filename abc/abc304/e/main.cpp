...
void solve() {
  ...
  int K; loadVar(K);
  VI X(K), Y(K); loadVec(K, X, Y);
  int L; loadVar(L);
  VI P(L), Q(L); loadVec(L, P, Q);

  dsu graph(N);
  rep(i, M)
    graph.merge(U[i] - 1, V[i] - 1);

  const auto& G = graph.groups();
  vector<int> roots(N);
  ...
  map<int, unordered_set<int>> constraints;
  rep(i, K) {
    int r_x = roots[X[i] - 1];
    int r_y = roots[Y[i] - 1];
    if(r_x > r_y) swap(r_x, r_y);
    constraints[r_x].insert(r_y);
  }
  
  rep(i, L) {
    int r_q = roots[Q[i] - 1];
    int r_p = roots[P[i] - 1];
    if(r_q > r_p) swap(r_q, r_p);
    PRINT_YN(!constraints[r_q].count(r_p));
  }
}
