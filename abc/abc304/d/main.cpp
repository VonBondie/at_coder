...
void solve() {
  LL W, H, N; loadVar(W, H, N);
  VLL P(N), Q(N); loadVec(N, P, Q);
  LL NA; loadVar(NA);
  VLL A(NA); loadVec(NA, A);
  LL NB; loadVar(NB);
  VLL B(NB); loadVec(NB, B);
  ...
  LL berry_min = LONG_LONG_MAX;
  LL berry_max = 0;
  LL has_berry_piece = 0;

  for(auto piece: pieces) {
    LL num = piece.second;
    berry_min = min(num, berry_min);
    berry_max = max(num, berry_max);
    has_berry_piece++;
  }

  if(has_berry_piece < (NA+1) * (NB+1)) berry_min = 0;

  cout << berry_min << " " << berry_max << endl;
}
