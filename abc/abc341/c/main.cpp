...
void solve() {
  ...
  S.resize(H);
  loadVec(H, S);
  int m_var = count(T.begin(), T.end(), 'D') - count(T.begin(), T.end(), 'U'); 
  int m_hor = count(T.begin(), T.end(), 'R') - count(T.begin(), T.end(), 'L'); 

  int var_start = max(1, 1 - m_var);
  int var_end = min(H-2, H-2-m_var);
  int hor_start = max(1, 1 - m_hor);
  int hor_end = min(W-2, W-2-m_hor);

  int ans = 0;

  for(int h = var_start; h <= var_end; h++) {
    for(int w = hor_start; w <= hor_end; w++) {
      ans += (check(h, w) ? 1 : 0);
    }
  }
