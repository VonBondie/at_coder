...
void solve() {
  ...
  rep(i, Q) {
    ...
    case 1:
      cin >> p1 >> p2;
      p1--;p2--;
      if(n_branches[p1] == 0) ans--; 
      if(n_branches[p2] == 0) ans--; 
      branches[p1].insert(p2);
      branches[p2].insert(p1);
      n_branches[p1]++;
      n_branches[p2]++;
    ...
    case
      2: cin >> p1;
      p1--;
      for(auto b: branches[p1]) {
        branches[b].erase(p1);
        n_branches[b]--;
        if(n_branches[b] == 0) ans++; 
      }
      if(n_branches[p1] != 0) ans++;
      branches[p1].clear();
      n_branches[p1] = 0;
