...
pair<int,int> getHead() {
  rep(i, H) {
    rep(j, W) {
      if(T[i][j] == -1) return pair<int, int>(i, j);
    }
  }
  return pair<int, int>(-1, -1);
}

bool fill_T(const int i, const int j, const pair<int, int> head) {
  const int A_ = (j==0 ? A[i] : B[i]);
  const int B_ = (j==0 ? B[i] : A[i]);

  const int head_h = head.first, head_w = head.second;

  rep(a, A_) {
    rep(b, B_) {
      if(T[head_h + a][head_w + b] != -1) return false;
      T[head_h + a][head_w + b] = i;
    }
  }
  return true;
}

void clear(const int i, const int j, const pair<int, int> head) {
  const int A_ = (j==0 ? A[i] : B[i]);
  const int B_ = (j==0 ? B[i] : A[i]);
  ...
  rep(a, A_) {
    rep(b, B_) {
      if(T[head_h + a][head_w + b] == i) T[head_h + a][head_w + b] = -1;
    }
  }
}

bool func(const char unused) {
  const auto head = getHead();
  if(head == pair<int, int>(-1, -1)) return true;
  const int head_h = head.first, head_w = head.second;
  ...
  rep(i, N) {
    ...
    rep(j, 2) {
      ...
      const int a = (j==0 ? A[i] : B[i]);
      const int b = (j==0 ? B[i] : A[i]);
      if(head_h + a > H)  continue;
      if(head_w + b > W)  continue;

      if(fill_T(i, j, head) && func(unused ^ (1<<i))) return true;
      clear(i, j, head);
    }
  }
